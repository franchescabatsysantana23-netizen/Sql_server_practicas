/*
SQL Server permite crear dos tipos de índices: 1) agrupados (clustered) y 2) no agrupados (nonclustered).
Un INDICE AGRUPADO es similar a una guía telefónica, los registros con el mismo valor de campo se agrupan juntos.
...
Un INDICE NO AGRUPADO es como el índice de un libro, los datos se almacenan en un lugar diferente al del índice,
...
Es recomendable crear los índices agrupados antes que los no agrupados, 
porque los primeros modifican el orden físico de los registros, ordenándolos secuencialmente.

SQL Server crea automaticamente índices cuando se crea una restricción "primary key" o "unique" en una tabla.
los índices facilitan la recuperación de datos, permitiendo el acceso directo y acelerando las búsquedas,
